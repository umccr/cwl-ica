// Author: Alexis Lucattini
// For assistance on generation of typescript expressions
// In CWL, please visit our wiki page at https://github.com/umccr/cwl-ica/wiki/TypeScript

// Imports
import {FileProperties as IFile} from "cwl-ts-auto";
import {File_class} from "cwl-ts-auto";
import {FastqListRow} from "../../../schemas/fastq-list-row/1.0.0/fastq-list-row__1.0.0";

// Backward compatibility with --target es5
declare global {
    interface Set<T> {
    }

    interface Map<K, V> {
    }

    interface WeakSet<T> {
    }

    interface WeakMap<K extends object, V> {
    }
}

export function get_fastq_list_rows_from_file(fastq_list_csv: IFile): Array<FastqListRow> {
    /*
    Load inputs initialise output variables
    Assumes only RG parameters set by bclconvert are RGID, RGSM, RGLB (in that order)
    */
    let output_array = [];

    // Assert that fastq list csv is defined and not null
    if (fastq_list_csv === undefined || fastq_list_csv === null) {
        throw new Error(`Could not get fastq list csv file object, got '${fastq_list_csv}' instead`)
    }

    // Math.pow(2, 16) is about 64 kb
    if (Math.pow(2, 16) < fastq_list_csv.size){
        throw new Error(`fastq_list.csv must be smaller than 64 Kb, but got a file of size ${fastq_list_csv.size} bytes!`);
    }

    if (fastq_list_csv.contents === undefined) {
        throw new Error("Could not read in fastq_list.csv file contents")
    }
    const lines = fastq_list_csv.contents.split("\n")

    /*
    Generate output object by iterating through fastq_list csv
    */
    let is_first_line = true;
    for (const line of lines) {
        if (is_first_line) {
            is_first_line = false;
            if (line.trim().indexOf("RGID,RGSM,RGLB,Lane,Read1File") !== 0) {
                throw new Error(`Could not confirm that the first line was at least started in the order of 'RGID,RGSM,RGLB,Lane,Read1File' got ${line} instead`);
            }
            continue
        }
        if (line.trim() === "") {
            /*
            Empty blank line at the end of the file
            */
            continue
        }
        let [rgid, rgsm, rglb, lane, read_1_path, read_2_path] = line.trim().split(",")

        /*
        Confirm none of rgid, rgsm, rglb, lane, and read_1_path are null
        */

        /*
        Initialise the output row as a dict
        */
        let output_fastq_list_row: FastqListRow = {
            rgid: rgid,
            rgsm: rgsm,
            rglb: rglb,
            lane: Number(lane),
            read_1: {
                "class_": File_class.FILE,
                "path": read_1_path
            }
        }

        if (read_2_path != null && read_2_path !== "") {
            /*
            read 2 path exists
            */
            output_fastq_list_row["read_2"] = {
                "class_": File_class.FILE,
                "path": read_2_path
            }
        }

        /*
        Append to row
        */
        output_array.push(output_fastq_list_row)
    }
    return output_array
}
