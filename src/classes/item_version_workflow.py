...
Never the less this class is used to handle the version attributes of an element in workflow.yaml, workflow.yaml etc.

This class is significantly more important than its owner Item.

Like Item, ItemVersion is only ever invoked through its subclasses ItemVersionWorkflow, ItemVersionWorkflow etc.
"""
...
class ItemVersionWorkflow(ItemVersion):
    """
    An ItemVersionWorkflow has the following properties
    ...
    * set_md5sum -> sets the md5sum attribute
    ...
    def set_cwl_object(self):
        ...
        # Split file name
        name, version = get_name_version_tuple_from_cwl_file_path(self.cwl_file_path, items_dir=get_workflows_dir())

        # Set here
        self.cwl_obj = CWLWorkflow(cwl_file_path=self.cwl_file_path, name=name, version=version)
