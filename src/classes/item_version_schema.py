...
Never the less this class is used to handle the version attributes of an element in workflow.yaml, schema.yaml etc.

This class is significantly more important than its owner Item.

Like Item, ItemVersion is only ever invoked through its subclasses ItemVersionSchema, ItemVersionWorkflow etc.
"""
...
class ItemVersionSchema(ItemVersion):
    """
    An ItemVersionSchema has the following properties
    ...
    * set_md5sum -> sets the md5sum attribute
    ...
    def set_cwl_object(self):
        ...
        # Split file name
        name, version = get_name_version_tuple_from_cwl_file_path(self.cwl_file_path, items_dir=get_schemas_dir())

        # Set here
        self.cwl_obj = CWLSchema(cwl_file_path=self.cwl_file_path, name=name, version=version)
